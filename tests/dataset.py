...
root=Path('test-results/integration')
...
b=zipfile.ZipFile(root/'built-rgb.zip')
data=rows(b)
...
summary={'rgb_frames':len(data),'cameras':images,'debug_overlay_pixels_equal':True,'depth_shape':shape,'valid_depth_pixels':int(valid.sum()),'instance_ids':list(map(int,np.unique(ids))),'memory_frames':len(memory),'wrong_choice_physical_success_separate':True}
