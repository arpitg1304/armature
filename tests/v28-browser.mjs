...
try {
  for (const file of [
    "armature-studio-v28.html",
    "dist/robotics-arm-studio.html",
  ]) {
    ...
    assert.equal(report.authoring.loaded.name, "V28 migration fixture");
    assert.deepEqual(
    ...
    if (file.startsWith("dist")) {
      ...
      assert.match(report.camera.error, /clone/);
    }
    console.log(
    ...
    if (file.startsWith("dist")) {
      ...
      await (await sensorDownload).saveAs("test-results/v28/sensors.zip");
    }
    ...
    const label = file.startsWith("dist") ? "built" : "reference";
    await fs.mkdir("test-results/v28", { recursive: true });
    await zip.saveAs("test-results/v28/" + label + "-rgb.zip");
    ...
    if (label === "built") {
      ...
      await page.evaluate(() => armature.download());
      await (await memoryDownload).saveAs("test-results/v28/memory.zip");
      console.log(
        "built: restored four memory protocols complete with correct choices",
      );
      ...
      await page.screenshot({ path: "test-results/v28/built.png" });
    }
  ...
  assert.deepEqual(
    reports[1].traces,
    reports[0].traces,
    "V28 simulation trajectories preserved",
  );
  assert.deepEqual(
    reports[1].camera.config,
    reports[0].camera.config,
    "V28 camera configuration preserved",
  );
  assert.deepEqual(
    reports[1].camera.snapshot,
    reports[0].camera.snapshot,
    "V28 camera calibration preserved",
  );
  assert.deepEqual(
    reports[1].teleop,
    reports[0].teleop,
    "V28 teleoperation takeover preserved",
  );
  await fs.writeFile(
    "test-results/v28/browser.json",
    JSON.stringify(reports, null, 2),
  );
  console.log(
    "PASS: V28/reference behavior parity, offline boot, authoring, sensors, recording, teleop and memory.",
  );
} finally {
  await browser.close();
}
